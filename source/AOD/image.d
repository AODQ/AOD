...
public:
  ...
  */
  ...
  /** Constructs sheet rect with a sheet container */
  this(SheetContainer sc, int ix, int iy, int width, int height) {
    ix *= width; iy *= height;
    this(sc, Vector(ix,       iy),
             Vector(ix+width, iy+height));
  }
}
...
SheetContainer Load_Image(const char* fil) @trusted {
  ...
  if ( ilLoadImage( fil ) == IL_TRUE ) {
    ...
    if ( !glIsTexture(GL_ID) ) {
      ...
      writeln("Error generating GL texture: ", fil);
      return SheetContainer();
    }
    ...
    import std.conv;
    import std.stdio : writeln;
    writeln("Error loading " ~ to!string(fil));
    writeln(to!string(iluErrorString(t)) ~ "(" ~ to!string(ilGetError()) ~ ")");
    assert(0);
  }
...
SheetContainer Load_Image(string fil) {
  return Load_Image(fil.toStringz);
}
